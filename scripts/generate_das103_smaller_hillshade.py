...
def main():
    ...
    smaller_clusters_path = "data/modified_das_cluster/smaller_das_cluster.gpkg"
    output_dir = Path("data/hillshade_clusters")
    output_dir.mkdir(parents=True, exist_ok=True)
    
    print("=" * 70)
    print("⛰️ GENERATING HILLSHADE FOR OPTIMIZED SMALLER CLUSTER DAS_103")
    print("=" * 70)
    print(f"📂 Source DEM       : {dem_path}")
    print(f"🎯 Boundary Source  : {smaller_clusters_path}")
    print(f"📁 Output Directory : {output_dir}")
    print("=" * 70)
    
    # Load smaller cluster GPKG
    gdf = gpd.read_file(smaller_clusters_path)
    col = 'ID_Cluster' if 'ID_Cluster' in gdf.columns else gdf.columns[0]
    
    # Filter DAS_103
    cluster_103 = gdf[gdf[col].isin([103, 103.0, '103', 'DAS_103'])]
    
    if cluster_103.empty:
        raise ValueError(f"Could not find DAS_103 in {smaller_clusters_path}")
        
    print(f"Found optimized DAS_103 polygon (Status: {cluster_103['Status'].values[0] if 'Status' in cluster_103.columns else 'OK'})")
    
    out_file = output_dir / "hillshade_DAS_103.tif"
    
    with rasterio.open(dem_path) as src:
        print("\n🌊 Processing DAS_103 (Optimized Smaller Boundary)...")
        w, h, size_mb, elapsed = process_cluster_streaming(src, cluster_103, out_file, chunk_size=2048)
    ...
    print("\n" + "=" * 70)
    print("🎉 OPTIMIZED DAS_103 HILLSHADE REGENERATED SUCCESSFULLY!")
    print("=" * 70)
